...
#drum_df -= np.mean(drum_df[-100:])
input_df -= np.mean(input_df[-100:])
...
def evaluate_params_on_full_dataset(params, start_idx, end_idx, use_measured_initial_position=True, make_plots=True):
    ...
    if make_plots:
        ...
        fit_drum_detr_corr = fit_percent(ys_full_detr[0], outputs_full_detr_corr[0])
...
full_evaluation_results = evaluate_params_on_full_dataset(params_test, start_idx_eval, end_idx_eval, use_measured_initial_position=True, make_plots=True)
