...
theta = theta - np.mean(theta)  # <-- zero-center the data to improve conditioning
...
axes[0].plot(t, theta,     label="θ(t)",  color="steelblue", linewidth=2)
axes[0].set_ylabel("θ  (rad)")
axes[0].legend()
axes[0].grid(True, alpha=0.3)

axes[1].plot(t, dtheta, label="dθ(t)", color="seagreen", linewidth=1.5)
axes[1].set_ylabel("dθ  (rad/s)")
axes[1].set_xlabel("Time  (s)")
axes[1].legend()
axes[1].grid(True, alpha=0.3)
 
axes[2].plot(d2theta, color="darkorange", linewidth=1.5)
axes[2].set_ylabel("d2θ  (rad/s²)")
...
x = least_squares(residuals_func, x0)
x = x.x
